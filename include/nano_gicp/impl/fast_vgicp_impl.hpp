...
FastVGICP<PointSource, PointTarget>::FastVGICP() : NanoGICP<PointSource, PointTarget>() {
...
void FastVGICP<PointSource, PointTarget>::setInputTarget(const PointCloudTargetConstPtr& cloud) {
  ...
  NanoGICP<PointSource, PointTarget>::setInputTarget(cloud);
  voxelmap_.reset();
}
...
void FastVGICP<PointSource, PointTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  ...
  NanoGICP<PointSource, PointTarget>::computeTransformation(output, guess);
}
